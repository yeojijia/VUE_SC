<template>
  <com-popup class="cash-popup" v-model:show="_show" :title="$t('defi.desc15')">
    <div class="content">
      <div class="con-center">
        <div class="title" v-html="info.name"></div>
        <div class="box">
          <div class="name">{{$t('defi.desc30')}}：</div>
          <div class="val">{{ decal(info.userPendingAmount, 3) }} {{ info.rewardCoin
            ?.symbol }}</div>
        </div>
        <div class="box">
          <div class="name">{{$t('defi.desc31')}}：</div>
          <div class="val">{{ info.userReceiveAmountAmount }} {{ info.rewardCoin?.symbol }}</div>
        </div>
      </div>
      <div class="con-bot">
        <div class="btn-con" @click="emit('confirm')">
          <loading-btn :loading="info.cashloading" :loading-text="$t('defi.desc32')" class="btn">
            {{ $t("defi.desc15") }}
          </loading-btn>
        </div>
      </div>
