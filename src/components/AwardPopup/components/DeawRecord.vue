<template>
  <div class="content">
    <div class="content-top">
      ...
      <div class="title">{{$t('award.desc7')}}</div>
    </div>
    <div class="content-table">
      <div class="table-head">
        <div class="tr">
          <div class="td">{{$t('award.desc9')}}</div>
          <div class="td">{{$t('award.desc10')}}</div>
          <div class="td">{{$t('award.desc11')}}</div>
        </div>
      </div>
...
import { _turntableRecord } from '@/utils/api/user'
...
const emit = defineEmits(['change', 'hide'])
const list = ref([])
...
onMounted(() => {
  ...
  _turntableRecord({ activityId: props.info.id })
    .then(res => {
      list.value = res
    }).finally(() => {
